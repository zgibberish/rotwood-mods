...
- Working:
    ...
    + Mod configuration options (**NOTE:** use `GetModConfigData(name, true)` instead, I couldn't figure out exactly why but not having `get_local_config` set to `true` will make it return nothing.)
